...
class Player : public Creature, public Cylinder
{
	public:
		...
		uint32_t getFrags() {return frags;}
		void setFrags(uint32_t newFrags) {frags = newFrags;}
		uint32_t getDeaths() {return deaths;}
		void setDeaths(uint32_t newDeaths) {deaths = newDeaths;}
		uint32_t getAssists() {return assists;}
		void setAssists(uint32_t newAssists) {assists = newAssists;}

		uint32_t getTmpFrags() {return fragsTmp;}
		void setTmpFrags(uint32_t newFrags) {fragsTmp = newFrags;}
		uint32_t getTmpDeaths() {return deathsTmp;}
		void setTmpDeaths(uint32_t newDeaths) {deathsTmp = newDeaths;}
		uint32_t getTmpAssists() {return assistsTmp;}
		void setTmpAssists(uint32_t newAssists) {assistsTmp = newAssists;}

		uint32_t getLastFrags() {return fragsLast;}
		void setLastFrags(uint32_t newFrags) {fragsLast = newFrags;}
		uint32_t getLastAssists() {return assistsLast;}
		void setLastAssists(uint32_t newAssists) {assistsLast = newAssists;}
