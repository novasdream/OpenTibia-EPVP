...
void LuaScriptInterface::registerFunctions()
{
	...
	//isWalking(cid)
	lua_register(m_luaState, "isWalking", LuaScriptInterface::luaIsWalking);
	...
	//getTime()
	lua_register(m_luaState, "getTime", LuaScriptInterface::luaGetTime);
	...
	//doPlayerSetTarget(cid, target)
	lua_register(m_luaState, "doPlayerSetTarget", LuaScriptInterface::luaDoPlayerSetTarget);
	...
	//getFrags(cid)
	lua_register(m_luaState, "getFrags", LuaScriptInterface::luaGetFrags);

	//setFrags(cid,value)
	lua_register(m_luaState, "setFrags", LuaScriptInterface::luaSetFrags);

	//getDeaths(cid)
	lua_register(m_luaState, "getDeaths", LuaScriptInterface::luaGetDeaths);

	//setDeaths(cid,value)
	lua_register(m_luaState, "setDeaths", LuaScriptInterface::luaSetDeaths);

	//getAssists(cid)
	lua_register(m_luaState, "getAssists", LuaScriptInterface::luaGetAssists);

	//setAssists(cid,value)
	lua_register(m_luaState, "setAssists", LuaScriptInterface::luaSetAssists);



	//getTmpFrags(cid)
	lua_register(m_luaState, "getTmpFrags", LuaScriptInterface::luaGetTmpFrags);

	//setTmpFrags(cid,value)
	lua_register(m_luaState, "setTmpFrags", LuaScriptInterface::luaSetTmpFrags);

	//getTmpDeaths(cid)
	lua_register(m_luaState, "getTmpDeaths", LuaScriptInterface::luaGetTmpDeaths);

	//setTmpDeaths(cid,value)
	lua_register(m_luaState, "setTmpDeaths", LuaScriptInterface::luaSetTmpDeaths);

	//getTmpAssists(cid)
	lua_register(m_luaState, "getTmpAssists", LuaScriptInterface::luaGetTmpAssists);

	//setTmpAssists(cid,value)
	lua_register(m_luaState, "setTmpAssists", LuaScriptInterface::luaSetTmpAssists);


	//getLastFrags(cid)
	lua_register(m_luaState, "getLastFrags", LuaScriptInterface::luaGetLastFrags);

	//setLastFrags(cid,value)
	lua_register(m_luaState, "setLastFrags", LuaScriptInterface::luaSetLastFrags);

	//getLastAssists(cid)
	lua_register(m_luaState, "getLastAssists", LuaScriptInterface::luaGetLastAssists);

	//setLastAssists(cid,value)
	lua_register(m_luaState, "setLastAssists", LuaScriptInterface::luaSetLastAssists);
	...
	//goTo(cid, position)
	lua_register(m_luaState, "goTo", LuaScriptInterface::luaGoTo);

	//canGoTo(cid, position)
	lua_register(m_luaState, "canGoTo", LuaScriptInterface::luaCanGoTo);
	...
	lua_register(m_luaState, "setPlayerModes", LuaScriptInterface::luaSetPlayerModes);

	//loadPlayer(name)
	lua_register(m_luaState, "loadPlayer", LuaScriptInterface::luaLoadPlayer);
...
int32_t LuaScriptInterface::internalGetPlayerInfo(lua_State* L, PlayerInfo_t info)
{
	...
	switch(info)
	{
		...
		case PlayerInfoManaSpent:
			...
			break;
		case PlayerInfoWalking:
			lua_pushboolean(L, player->isWalking());
			return 1;
		case PlayerInfoTown:
...
int32_t LuaScriptInterface::luaIsWalking(lua_State* L)
{
	return internalGetPlayerInfo(L, PlayerInfoWalking);
}
...
int32_t LuaScriptInterface::luaGetFrags(lua_State* L)
{
	//getFrags(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
...
int32_t LuaScriptInterface::luaSetFrags(lua_State* L)
{
	//setFrags(cid, value)
...
int32_t LuaScriptInterface::luaGetDeaths(lua_State* L)
{
	//getDeaths(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getDeaths());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetDeaths(lua_State* L)
{
	//setDeaths(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetAssists(lua_State* L)
{
	//getAssists(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getAssists());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetAssists(lua_State* L)
{
	//setAssists(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetTmpFrags(lua_State* L)
{
	//getTmpFrags(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getTmpFrags());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetTmpFrags(lua_State* L)
{
	//setTmpFrags(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetTmpDeaths(lua_State* L)
{
	//getTmpDeaths(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getTmpDeaths());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetTmpDeaths(lua_State* L)
{
	//setTmpDeaths(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetTmpAssists(lua_State* L)
{
	//getTmpAssists(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getTmpAssists());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetTmpAssists(lua_State* L)
{
	//setTmpAssists(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetLastFrags(lua_State* L)
{
	//getLastFrags(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getLastFrags());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetLastFrags(lua_State* L)
{
	//setLastFrags(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaGetLastAssists(lua_State* L)
{
	//getLastAssists(cid)
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(Player* player = creature->getPlayer())
		{
			lua_pushnumber(L, player->getLastAssists());
		}
		else
		{
			lua_pushnumber(L, -1);
...
int32_t LuaScriptInterface::luaSetLastAssists(lua_State* L)
{
	//setLastAssists(cid, value)
	uint32_t value = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaScriptInterface::luaDoPlayerSetTarget(lua_State* L)
{
	//doPlayerSetTarget(cid, target)
	...
	lua_pushboolean(L, player->setAttackedCreature(target));
...
int32_t LuaScriptInterface::luaGetTime(lua_State* L)
{
	//getTime()
...
{
	//goTo(cid, position)
	uint32_t nodesLimit = 100;
	if(lua_gettop(L) > 7)
		nodesLimit = popNumber(L);
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
	     if(g_game.getPathToEx(player, pos, listDir, minDist, maxDist, fullPathSearch, clearSight, maxSearchDist, nodesLimit))
	     {
...
int32_t LuaScriptInterface::luaLoadPlayer(lua_State* L)
{
	...
	if(players.empty())
	{
    	...
    	player->setClientVersion(666);
    	lua_pushboolean(L, true);
	    //lua_pushboolean(L, connect(_player->getID(), CLIENTOS_WINDOWS, 666));
