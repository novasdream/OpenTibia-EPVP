...
uint32_t CreatureEvent::executeLogin(Player* player)
{
	...
	if(m_interface->reserveEnv())
	{
		...
		else if(m_scripted == EVENT_SCRIPT_TRUE)
		{
			#ifdef __DEBUG_LUASCRIPTS__
			char desc[35];
...
uint32_t CreatureEvent::executeAppear(Creature* creature, const Creature* target)
{
	//onAppear(cid, target)
	if(m_interface->reserveEnv())
	{
        Creature* nc_target = const_cast < Creature* > (target);
		ScriptEnviroment* env = m_interface->getEnv();
		if(m_scripted == EVENT_SCRIPT_BUFFER)
		{
			env->setRealPos(creature->getPosition());
			...
			scriptstream << "local cid = " << env->addThing(creature) << std::endl;
			scriptstream << "local target = " << env->addThing(nc_target) << std::endl;
		...
		else
		{
			...
			lua_pushnumber(L, env->addThing(creature));
			lua_pushnumber(L, env->addThing(nc_target));
	...
	{
		std::cout << "[Error - CreatureEvent::executeAppear] Call stack overflow." << std::endl;
		return 0;
	}
}


uint32_t CreatureEvent::executeDisappear(Creature* creature, const Creature* target)
{
	//onDisppear(cid, target)
	if(m_interface->reserveEnv())
	{
        Creature* nc_target = const_cast < Creature* > (target);
		ScriptEnviroment* env = m_interface->getEnv();
