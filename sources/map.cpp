...
bool Map::getPathMatching(const Creature* creature, std::list<Direction>& dirList,
	const FrozenPathingConditionCall& pathCondition, const FindPathParams& fpp, uint32_t nodesLimit /*= 100 */)
{
	...
	const Tile* tile = NULL;
	while(fpp.maxSearchDist != -1 || nodes.countClosedNodes() < nodesLimit)
	{
		if(!(n = nodes.getBestNode()))
		{
