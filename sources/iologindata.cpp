...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	query << "`health`, `healthmax`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, ";
	query << "`lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, ";
	query << "`lastlogout`, `lastip`, `conditions`, `guildnick`, `rank_id`, `town_id`, ";
	query << "`balance`, `kills`, `deaths`, `assists`, `marriage`, `promotion`, `description` FROM `players` WHERE `name` ";
	...
	player->frags = result->getDataLong("kills");
	player->deaths = result->getDataLong("deaths");
	player->assists = result->getDataLong("assists");
...
bool IOLoginData::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	query << "`kills` = " << player->frags << ", ";
	query << "`deaths` = " << player->deaths << ", ";
	query << "`assists` = " << player->assists << ", ";
...
bool IOLoginData::saveItemBinary(PropWriteStream& stream, const Item* item)
{
...
bool IOLoginData::playerDeath(Player* _player, const DeathList& dl)
{
	...
	for(DeathList::const_iterator it = dl.begin(); i < size && it != dl.end(); ++it, ++i)
	{
		...
		query << "INSERT INTO `killers` (`death_id`, `final_hit`, `unjustified`"
			...
			<< ", `war`"
#endif
			<< ") VALUES (" << deathId << ", " << (it == dl.begin()) << ", " << it->isUnjustified();
#ifdef __WAR_SYSTEM__
		if(it == dl.begin()) //last hit is always first and we got stored war data only there
		{
			War_t tmp = it->getWar();
