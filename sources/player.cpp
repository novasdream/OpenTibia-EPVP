	...
	Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	frags = deaths = assists = 0;
	fragsTmp = deathsTmp = assistsTmp = 0;
	fragsLast = assistsLast = 0;
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	else
	{
		...
		s << ". " << (sex % 2 ? "He" : "She") << " has " << fragsTmp << " frags";
	}
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if(player->getTile()->ground && player->getTile()->ground->getID() == ITEM_GLOWING_SWITCH)
		return false;
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if(creature != this)
	{
		...
		{
        	CreatureEventList appearEvents = getCreatureEvents(CREATURE_EVENT_APPEAR);
        	for(CreatureEventList::iterator it = appearEvents.begin(); it != appearEvents.end(); ++it)
        		(*it)->executeAppear(this, creature);
        }
...
bool Player::setAttackedCreature(Creature* creature)
{
	...
	{
		sendCancelTarget();
		std::cout << "Player::setAttackedCreature false" << std::endl;
		return false;
	}
...
void Player::getPathSearchParams(const Creature* creature, FindPathParams& fpp) const
{
	...
	if((vocation_id == 1 || vocation_id == 2 || vocation_id == 3) && clientVersion == 666)
	{
		fpp.minTargetDist = 2;
		fpp.maxTargetDist = 2 + (int) (((time(NULL) + guid) % 30) / 10);
	}
}
...
bool Player::isEnemy(const Player* player, bool allies) const
{
	...
	return !warMap.empty() && ((allies && guildId == guild) ||
...
bool Player::onKilledCreature(Creature* target, uint32_t& flags, DeathEntry& entry)
{
	if(!Creature::onKilledCreature(target, flags, entry))
		return false;
	...
	War_t enemy;
	if(targetPlayer->getEnemy(this, enemy) && (!hasBitSet((uint32_t)KILLFLAG_LASTHIT, flags) || IOGuild::getInstance()->updateWar(enemy)))
		entry.setWar(enemy);
...
{
	std::string strValue;
	if(getStorage(59432, strValue) && atoi(strValue.c_str()) > 0)
		return false;
	if(checkInfight && getZone() != ZONE_PROTECTION && hasCondition(CONDITION_INFIGHT))
		return false;
...
uint32_t Player::getAttackSpeed()
{
	...
	if(weapon && weapon->getAttackSpeed() != 0)
		return vocation->getAttackSpeed() * weapon->getAttackSpeed() / 1000;
